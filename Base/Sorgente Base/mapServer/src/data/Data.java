...
public class Data {
	...
	public Data(String tableName) throws TrainingDataException {
			...
			try {
				...
				throw new TrainingDataException("The table doesn't exists!");
			}
			...
			try {
				...
				throw new TrainingDataException("The table has 0 rows");
			}
			...
			for( i=0; i<ts.getNumberOfAttributes()-1; i++) {
				...
				if(ts.getColumn(i).isNumber()) {
					...
					try {
						...
						throw new TrainingDataException("Error in the reception of distinct values");
					}catch(NullPointerException e1) {
						throw new TrainingDataException("Found a null value in a row");
					}
			...
			if(ts.getColumn(ts.getNumberOfAttributes()-1).isNumber()) {
				...
				throw new TrainingDataException("The last column is not a continuous value");
			}
